function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for name, pos in pairs(required_vocations) do
        ...
        if player_vocation:getName() == name and player_pos.x == pos[1] and player_pos.y == pos[2] and player_pos.z == pos[3] then
            table.insert(players_to_teleport, player)
        else
            player:say("You do not meet the requirements to use this teleport.", TALKTYPE_MONSTER_SAY)
            return true
        end
    end
    ...
    if all_items_present and #players_to_teleport == 4 then
